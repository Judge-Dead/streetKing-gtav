...
AddEventHandler('esx_phone:loaded', function(phoneNumber, contacts)
	...
	TriggerEvent('esx_phone:registerMessageCallback', 'reply', function(sender, phoneNumber, type, message)
	
        TriggerEvent('chatMessage', 'BPOST', {0, 255, 0}, sender .. ' : ' .. message)
...
AddEventHandler('esx_phone:addContact', function(name, phoneNumber, type, notif)
	...
	if notif then
		...
		--TriggerEvent('es_freeroam:notify', user, 'CHAR_ANDREAS', {255,0,0} 'Contact ajouté')
...
AddEventHandler('esx_phone:onMessage', function(phoneNumber, playerName, type, message, actions)

	-- TriggerEvent('es:showNotification', playerName .. ' : ' .. message)
	TriggerEvent('chatMessage', 'BPOST', {0, 255, 0}, playerName .. ' : ' .. message)
...
RegisterNUICallback('message_callback', function(data, cb)
	...
	for i=1, #RegisteredMessageCallbacks[data.action], 1 do
		RegisteredMessageCallbacks[data.action][i](data.sender, data.phoneNumber, data.type, data.message);
	end
...
RegisterNUICallback('send', function(data, cb)
	...
	TriggerEvent('chatMessage', 'BPOST', {0, 255, 0}, 'Votre Lettre � bien ete envoyer')
